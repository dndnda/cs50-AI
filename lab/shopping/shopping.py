...
def load_data(filename):
    ...
    with open(filename, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            ev.append(int(month[row["Month"]]))
            ev.append(int(row["OperatingSystems"]))
            ev.append(int(row["Browser"]))
            ev.append(int(row["Region"]))
            ev.append(int(row["TrafficType"]))
            ev.append(1 if row["VisitorType"] == "Returning_Visitor" else 0)
            ev.append(1 if row["Weekend"] == "TRUE" else 0)
            evidence.append(ev)
...
def evaluate(labels, predictions):
    ...
    """
    pos_all, pos_right, neg_all, neg_right = 0,0,0,0
    ...
    for i, j in zip(labels, predictions):
        if i == 0:
            ...
            raise TypeError
    print(pos_right,pos_all, neg_right,neg_all)
    return (pos_right/pos_all, neg_right/neg_all)
